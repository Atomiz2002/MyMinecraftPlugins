...
public class Swat extends EntityPigZombie {
	...
	private static org.bukkit.inventory.ItemStack[] equip = Settings.getSwatEquip();
...
//			cField.set(targetSelector, new UnsafeList<PathfinderGoalSelector>());
		...
		setCustomName(Settings.getSwatName());
...
//		}
		...
		goalSelector.a(3, new PathfinderGoalMoveIndoors(this));
...
//				levels.add(CrimeManager.getWantedLevel((Player) ent), ent);
//			else
//				return;
//
...
//		goalSelector.a(2, new PathfinderGoalMeleeAttack((EntityCreature) levels.get(levels.size() - 1), 1.0D, false));
		...
		/* Setting the pigman equipment */
		EntityEquipment e = ((PigZombie) getBukkitEntity()).getEquipment();
		e.setHelmet(equip[0]);
		e.setChestplate(equip[1]);
		e.setLeggings(equip[2]);
		e.setBoots(equip[3]);
		e.setItemInMainHand(equip[4]);

		/* adding the pigman to the policemen list */
		Main.policeUUIDs.add(getUniqueID());
	}

	/* changing the pigman attributes */
	@Override
